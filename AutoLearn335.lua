...
local TEACH_RIDING = false  --> Decide si enseñar equitación o no. Paladines y Bujos aprenden su primer caballo sin el primer riding, para el segundo necesitan los dos.
...
local stopXP = 0 -- NO TOCAR ESTE VALOR
...
local function do_quest(pl, qid)             --> Función que llamaremos para completar misiones.
    if not pl:GetQuestRewardStatus(qid) then --> Comprueba que el jugador no haya realizado la misión.
        ...
        stopXP = 1                           --> Deshabilitamos la ganancia de XP por un instante.
        pl:RewardQuest(qid)                  --> Entregar misión y recibir recompesas; (xp, reputación, dinero, ítems y/o spells).
        stopXP = 0                           --> Volvemos a habilitar la ganancia de XP.
    end
end
...
local function al_subirNivel(e, P, old) ---------------------------------------------------------------------------------------
    ...
    if SCRIPT_ON then
        ...
        if (C == 64) and (DO_QUESTS == false) then --> Tótems de Chamanes, se ejecuta solo si DO_QUESTS esta false.
            if L == 4 or L == 10 or L == 20 or L == 30 then
                ...
                if P:AddItem(totems[L], 1) then
                    ...
                    P:SendAreaTriggerMessage("|cffff0000¡Inventario lleno! |rItem: " .. itemLink ..
                        " enviado al correo.")
        ...
        local function TEACH_SKILLS(class, level)                     --> Aquí recuperamos los valores de las IDs para luego aprenderlas.
            local t_name = A and "aa_autolearn_a" or "aa_autolearn_h" --> Me gusta más el operador ternario.
            local query = string.format("SELECT `skill` FROM `" .. t_name .. "` WHERE `level` <= %d AND `class` = %d",
                level, class)
            local SQL = WorldDBQuery(query)
            local skills = {} --> Declaramos nuestra tablita.

            if SQL then
                table.insert(skills, SQL:GetUInt32(0)) --> No sé por qué pero dentro del while no se inserta el primer valor. Así que lo hago antes.
                while SQL:NextRow() do
                    table.insert(skills, SQL:GetUInt32(0))
                end
        ...
        if DO_QUESTS then
            ...
            if questList then --> Bucle para hacer misiones.
                for _, quest in ipairs(questList) do
                    do_quest(P, quest)
                    --P:SendBroadcastMessage("|cff00ff00---Se completó una misión---")
                end
        ...
        local function Msg(pl, te)
            pl:SendAreaTriggerMessage(te)
            pl:SendBroadcastMessage(te)
        end

        if C == 1 then --> Guerrero
            if L == 10 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.actdef) end
                P:RegisterEvent(timed, t)
            elseif L == 30 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.actrab) end
                P:RegisterEvent(timed, t)
            end
        elseif C == 2 then --> Paladín
            if L == 12 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.reden) end
                P:RegisterEvent(timed, t)
            end
        elseif C == 4 then --> Cazador
            if L == 10 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.domes) end
                P:RegisterEvent(timed, t)
            end
        elseif C == 64 then --> Chamán
            if L == 4 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.tierra) end
                P:RegisterEvent(timed, t)
            elseif L == 10 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.fuego) end
                P:RegisterEvent(timed, t)
            elseif L == 20 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.agua) end
                P:RegisterEvent(timed, t)
            elseif L == 30 then
        ...
        elseif C == 256 then --> Brujo
            if L == 10 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.abis) end
                P:RegisterEvent(timed, t)
            elseif L == 20 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.sucu) end
                P:RegisterEvent(timed, t)
            elseif L == 30 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.manaf) end
                P:RegisterEvent(timed, t)
            end
        elseif C == 1024 then --> Druida
            if L == 10 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.oso) end
                P:RegisterEvent(timed, t)
            elseif L == 14 then
                local function timed(Ev, del, rep, OB) Msg(OB, s.p1 .. s.enven) end
                P:RegisterEvent(timed, t)
            end
        end
    end
...
local function al_logear(e, P)
    ...
    if SCRIPT_ON and SCRIPT_SHOW then P:SendBroadcastMessage("|cff00ff00HechizosAutomáticos|r funcionando.") end
end
...
local function elunaLoad(ev) --> Se ejecuta cuando se inicia el servidor, o se le hace reload al ElunaEngine
    ...
    local db = { --> Preparamos la consulta para crear las bases de datos necesarias.
        a = cr .. "`aa_autolearn_a` (`level` TINYINT(3)" .. u ..
            ",`class` SMALLINT(4)" .. u .. ",`skill` MEDIUMINT(6)" .. u .. ")",
        h = cr .. "`aa_autolearn_h` (`level` TINYINT(3)" .. u ..
            ",`class` SMALLINT(4)" .. u .. ",`skill` MEDIUMINT(6)" .. u .. ")"
    }
...
local function al_ganar_xp(e, P, amount, victim, source)
    if stopXP == 1 then
        return 0 --> Este es el control para detener la ganancia de XP.
    end
end
...
RegisterServerEvent(33, elunaLoad) --> Esto crea las DB necesarias, solo se necesita ejecutar una vez, puedes desactivarlo si ya se ejecutó.
RegisterPlayerEvent(12, al_ganar_xp)
